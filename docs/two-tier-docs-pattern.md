...
This is the same pattern operating systems use (kernel API surface vs. man pages), and that the open-source world has converged on (`README.md` for orientation, `docs/` for substance).
...
[`docs/automation-decision-framework.md`](docs/automation-decision-framework.md).
```

Tier 2 → Tier 1: assume the reader has read it.

```markdown
# docs/automation-decision-framework.md
...
- Root file balloons.
- Concepts duplicated between root and `docs/`.
...
- **Claude Code** loads `CLAUDE.md` automatically. Keep it short; deep references in `docs/` are loaded on demand by the agent reading them.
