...
<!--
Frontmatter is per-vendor - this block is Claude Code's shape. Before using it elsewhere:

  Claude Code   keep the space-separated `allowed-tools` scalar (or use a YAML list);
                `Agent`, not the retired `Task` name, is the current subagent tool.
  Codex         keep only the verified portable `name` + `description` baseline unless
                the installed Codex documentation establishes additional fields.
  Gemini/Kiro/Cursor  keep only `name` + `description`.
  Devin Desktop       `allowed-tools` is a YAML list of lowercase ids (read, grep, glob, exec).

Declaring a tool the runtime does not have is the most common way this skill silently
fails to orchestrate anything. See runtimes/.<vendor>/skills/all-hands/ for a working
per-vendor frontmatter for each.
-->
