#!/usr/bin/env bash
# Kiro SessionStart hook: prints a compact SpecRoute orientation banner.
# Informational only - always exits 0.
#
# Kiro accepts either plain text on stdout (used as context) or Claude-compatible
# JSON. This emits the JSON form so the banner lands in the model's context.

set -u

# The script lives at <workspace>/.kiro/hooks/scripts/, so three levels up is
# the workspace root. Kiro runs hook commands from the workspace root, so the relative command
