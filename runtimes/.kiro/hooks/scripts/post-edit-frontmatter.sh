#!/usr/bin/env bash
# Kiro PostToolUse hook: validates SpecRoute frontmatter contracts after a
# file write. Warns and never blocks.
#
# Registered with an empty matcher for the same reason as the PreToolUse gate. The
# warning is returned as Claude-compatible additionalContext so the agent sees it.
...
case "$FILE_PATH" in
  */.claude/agents/*.md|*/.kiro/agents/*.md|*/.cursor/agents/*.md|*/.gemini/agents/*.md|*/agents/examples/*.md|*/agents/agent-template.md)
    ARTIFACT="agent" ;;
