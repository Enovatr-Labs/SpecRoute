#!/usr/bin/env bash
# Gemini CLI BeforeTool hook: blocks publish-style commands (git commit, git push,
...
#
# Registered with an empty matcher on purpose - Gemini matches BeforeTool on the
# tool name (run_shell_command and any MCP equivalent), and a matcher that fails
# to match is a silent hole. Exit 2 is a system block; stderr is the reason.
...
')"
[ "$MODE" != "none" ] || { python3 -c 'import json; print(json.dumps({"continue": True}))'; exit 0; }
...
WORDLIST=""
for candidate in "$REPO_ROOT/.gemini/.forbidden-strings.txt" "$REPO_ROOT/.claude/.forbidden-strings.txt"; do
  if [ -f "$candidate" ]; then
    WORDLIST="$candidate"
    break
  fi
