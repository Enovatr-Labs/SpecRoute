#!/usr/bin/env bash
# Cursor afterFileEdit hook: validates SpecRoute frontmatter contracts after a
# file write. Warns and never blocks.
#
# afterFileEdit is observational - Cursor ignores its output, so the warning goes
# to stderr and shows up in the hook log rather than in the chat transcript.
