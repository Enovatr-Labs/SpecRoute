#!/usr/bin/env bash
# Cursor beforeShellExecution hook: blocks publish-style commands (git commit, git push,
...
#
# beforeShellExecution is a control hook: it returns a permission decision. This
# script denies via BOTH the JSON decision and exit code 2, and the hook entry
# sets failClosed: true - so a crash blocks too. Cursor is fail-open by default.
...
import re, sys
text = sys.stdin.read(); prefix = r"(?:^|[;&|])\s*(?:[A-Za-z_]\w*=\S+\s+)*(?:command\s+)?"
...
')"
[ "$MODE" != "none" ] || { python3 -c 'import json; print(json.dumps({"permission": "allow"}))'; exit 0; }
...
WORDLIST=""
for candidate in "$REPO_ROOT/.cursor/.forbidden-strings.txt" "$REPO_ROOT/.claude/.forbidden-strings.txt"; do
  if [ -f "$candidate" ]; then
    WORDLIST="$candidate"
    break
  fi
...
if [ -z "$WORDLIST" ]; then
  python3 -c 'import json; print(json.dumps({"permission": "allow"}))'; exit 0
fi
...
while IFS= read -r line || [ -n "$line" ]; do
  ...
  while IFS= read -r hit_path; do [ -n "$hit_path" ] && HITS+=("$term -> worktree:$hit_path"); done <<< "$matched"
  staged="$(git grep --cached -l -i -- "$term" 2>/dev/null || true)"
  while IFS= read -r hit_path; do [ -n "$hit_path" ] && HITS+=("$term -> index:$hit_path"); done <<< "$staged"
  if [ "$MODE" = "publish" ]; then
    ...
    historical=""; [ -z "$revisions" ] || historical="$(git grep -l -i -- "$term" $revisions -- 2>/dev/null || true)"
    while IFS= read -r hit_path; do [ -n "$hit_path" ] && HITS+=("$term -> outgoing:$hit_path"); done <<< "$historical"
  fi
done < "$WORDLIST"
