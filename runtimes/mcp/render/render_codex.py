...
Usage:
    ...
    uv run runtimes/mcp/render/render_codex.py > runtimes/.codex/config.template.toml
...
HEADER = """\
# Codex configuration template. Drop into .codex/config.toml in your repo.
# This file is tracked. Per-user secrets belong in environment variables.
# Generated from runtimes/mcp/servers.yaml. Do not edit by hand.
# Re-run runtimes/mcp/render/render_codex.py to regenerate.
...
def render(servers: list[dict]) -> str:
    ...
    for s in servers:
        ...
        # TOML has real comments, so unlike the JSON renderers this one can state
        # the `requires_env` requirement inline without a pseudo-key. Codex does
        # accept `env = { KEY = "value" }`, but values are literal - emitting one
        # here would either commit a secret or hand the server a placeholder
        # string, so the requirement is documented instead. Never dropped.
        note = env_note(s)
        if note:
            parts.append(f"# {note}")
        for i, (tool, mode) in enumerate((s.get("codex_tool_approvals") or {}).items()):
            parts.append("")
            if i == 0:
                parts.append(f"# Per-tool approval mode for {name} operations:")
