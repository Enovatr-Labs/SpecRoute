...
- **Pointers to authoritative external state**: the user-level memory directory at `~/.claude/projects/<flattened-project-path>/memory/`, which holds the canonical sanitization rules. See [Resolving the user-level memory path](#resolving-the-user-level-memory-path) below.
...
- Information that belongs in user-level memory (cross-project facts about the user) - those live in `~/.claude/projects/<flattened-project-path>/memory/` and stay out of git.

## Resolving the user-level memory path

Claude Code stores per-project user-level state under `~/.claude/projects/`, in a directory whose name is the project's absolute path with every `/` replaced by `-`. Never hard-code a resolved path in a tracked file - it embeds the maintainer's real home directory, which the `/sanitize` flattened-path check treats as a leak. Derive it at runtime instead:

```bash
# Prints the user-level memory dir for the current repo, no hard-coded home dir.
echo "$HOME/.claude/projects/$(git rev-parse --show-toplevel | tr '/' '-')/memory"
```

Refer to it in prose as `~/.claude/projects/<flattened-project-path>/memory/`.
