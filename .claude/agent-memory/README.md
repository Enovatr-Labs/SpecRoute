...
Per-agent persistent context for the SpecForge implementation team. Each subdirectory matches a `.claude/agents/<name>.md` agent and holds free-form markdown notes the agent should re-read at the start of each conversation.
...
- **Pointers to authoritative external state**: the user-level memory directory at `~/.claude/projects/-Users-chika-LocalDev-SpecForge/memory/`, which holds the canonical sanitization rules.
