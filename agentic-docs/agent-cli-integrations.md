...
This matrix appears in four places (`README.md`, `AGENTS.md`, `wiki/Vendor-Matrix.md`, this file) and must stay in lock-step. The table body and its footnote block are byte-identical in all four - copy the whole block rather than editing a single cell. All six tools ship the same capability *classes*; that is not the same as convergence (see [Capability convergence](#vendor-capability-convergence) below).
