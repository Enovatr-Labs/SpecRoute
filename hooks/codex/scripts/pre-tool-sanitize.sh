#!/usr/bin/env bash
# Codex PreToolUse hook: blocks publish-style commands (git commit, git push,
...
#
# Registered with an empty matcher on purpose. Codex matches PreToolUse on the tool
...
WORDLIST=""
for candidate in "$REPO_ROOT/.codex/.forbidden-strings.txt" "$REPO_ROOT/.claude/.forbidden-strings.txt"; do
  if [ -f "$candidate" ]; then
    WORDLIST="$candidate"
    break
  fi
