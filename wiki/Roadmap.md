...
- ✓ `runtimes/.gemini/` — settings.json + `.gemini/commands/*.toml` templates
- ✓ `runtimes/.kiro/` — steering, specs, hooks
- ✓ `runtimes/.cursor/rules/` — `.mdc` templates
- ✓ `runtimes/.windsurf/rules/`
- ✓ `runtimes/mcp/servers.yaml` + six per-vendor renderers (claude, codex, gemini, kiro, cursor, windsurf)
