...
SpecForge exists because **agent-assisted development drifts when there is no contract for what is being built.** This page captures the operating beliefs that shape every template, prompt, and convention.

For the canonical version with full prose, see [`agentic-docs/philosophy.md`](https://github.com/Enovatr-Labs/SpecForge/blob/main/agentic-docs/philosophy.md).
...
Tooling churns. SpecForge's value is **content** — PRDs, specs, prompts, agents, rules, skills, commands, hooks. The runtime layout is a thin shell adapting that content to each vendor. A consumer can switch vendors without rewriting their PRDs and specs; only the runtime shell changes. See [[Vendor Matrix]].
...
SpecForge is open-source, derived from internal codebases. Patterns generalize freely; product names, customer data, and proprietary domain logic do not. See [[Sanitization]].
...
- **Build tooling, test runners, app frameworks.** SpecForge is markdown content. Scripts exist only when load-bearing (sanitization, cross-vendor sync, MCP rendering).
...
- Not a standards body. SpecForge proposes patterns; consumers fork what they need and ignore the rest.
