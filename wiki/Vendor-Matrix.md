...
- Don't drift the matrix between `README.md`, `AGENTS.md`, `agentic-docs/agent-cli-integrations.md`, and this page - when one changes, all four change. The table body and its footnote block are byte-identical across all four by design; copy the whole block rather than editing one cell.
